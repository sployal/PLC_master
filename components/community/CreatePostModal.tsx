...
interface Props {
  ...
  newPostCategory: 'help' | 'project' | 'discussion' | '';
  setNewPostCategory: (v: 'help' | 'project' | 'discussion' | '') => void;
  newPostImage: string;
    ...
    <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
      <div className="bg-[#1C1F26] border border-[#2D3138] rounded-2xl w-full max-w-2xl p-6 md:p-8 relative max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        <div>
          ...
          />
          {formErrors.title && <p className="text-red-400 text-xs mb-3 px-1">⚠️ Please enter a title</p>}
          {!formErrors.title && <div className="mb-3" />}
        ...
        <style>{`
          ...
          .custom-scrollbar::-webkit-scrollbar-thumb { background: #4a5568; border-radius:7px; border:3px solid rgba(0,0,0,0.3); }
          .custom-scrollbar::-webkit-scrollbar-thumb:hover { background:#5a6678; }
        `}</style>
      </div>
    </div>
  );
